...
Quantum Field Theory (QFT) is a theoretical framework that combines quantum mechanics and special relativity to describe the behavior of elementary particles and their interactions. It treats particles as excitations of underlying quantum fields that pervade all of spacetime. In QFT, fields are quantized, meaning they are described by operators that create and annihilate particles. The theory provides a mathematical framework for calculating probabilities of particle interactions. In fact, QFT is the consequence of Heisenberg Uncertainty Principle (Energy-Time) in the exact sense of a minimum frequency/energy of a quantum (Emin = hfmin) which is the Zero-Point energy where particles only exist as excitations in second-quantized quantum fields (matter fields specifically) insofar that such excitations exceed the minimum frequency of a quantum from the Planck energy formula. 
One of the most important equations governing QFT is the famous Dirac equation that tries to capture relativistic effects and high-energy in quantum mechanics. It is thanks to the Dirac equation (and Paul Dirac works at large) that we came to discover a crucial property of the universe that serves as a pillar to quantum field theory which is the existence of antimatter as allowed by the Dirac equation. The Dirac equation is basically the special-relativistic version of the Energy-Momentum relation (Einstein-Pythagoras relation). Moreover, the Dirac equation is a rectification of the Klein-Gordon equation as an attempt to solve the negative-energy problem associated with the Klein-Gordon equation that, for the context, tried to achieve the same thing via combining special relativity and energy-momentum relation except applied to spin-0 particles. The Dirac equation is basically a relativistic wave equation (combining particle/high-energy physics, quantum mechanics and special relativity) applied to Dirac particles (spin-1/2 massive particles). It describes wave functions in a set of 4 complex numbers called bispinors (as opposed to Schrodinger equation that includes only 1 complex value). Another variant of the Dirac equation is the Weyl equation in the case of zero mass. 
...
## Quantum Tunneling, Zeno effect, Entanglement and Superposition
Quantum tunneling refers to the phenomenon in quantum mechanics where a particle can pass through a potential barrier, even if it does not have enough energy to overcome it classically. This occurs because, according to the principles of quantum mechanics, particles also exhibit wave-like properties. As a result, there is a finite probability for the particle to "tunnel" through the barrier and appear on the other side, defying classical expectations.

The Quantum Zeno effect is a phenomenon named after Greek philosopher Zeno of Elea. It describes the idea that continuous observation or measurement of a quantum system can prevent its evolution or change. According to quantum mechanics, the act of measurement can collapse the system's wavefunction into a specific state. When measurements are frequently and rapidly performed on a system, it can effectively remain in a particular state, appearing frozen or unaffected by time evolution.

Quantum superposition is a fundamental principle in quantum mechanics that states that particles or systems can exist in multiple states simultaneously. In other words, a quantum system can be in a superposition of different possible states, with each state having a certain probability associated with it. This concept is often illustrated through the famous example of Schrödinger's cat, which can be simultaneously alive and dead until it is observed or measured, collapsing the superposition into a single state.

Quantum entanglement is a phenomenon where two or more particles become correlated in such a way that their quantum states are intertwined and dependent on each other, regardless of the distance between them. When particles are entangled, measuring the state of one particle instantaneously determines the state of the other, even if they are separated by large distances. This peculiar correlation exists because entangled particles share a single quantum state, rather than having individual states. Einstein referred to this phenomenon as "spooky action at a distance," as it challenges our classical understanding of locality and the concept of information transfer. Quantum entanglement is a key resource in quantum information and quantum computing, enabling tasks such as secure communication and quantum teleportation.
